...
if [ "$SOCIALECHO_BASE" != "$SOCIALECHO_HOME" ] && [ -r "$SOCIALECHO_BASE/socialbus-server.jar" ] ; then
  CLASSPATH="$CLASSPATH""$SOCIALECHO_BASE"/socialbus-server.jar
else
  CLASSPATH="$CLASSPATH""$SOCIALECHO_HOME"/socialbus-server.jar
fi
