...
CLASSPATH="$SOCIALECHO_BASE"/conf
...
if [ "$SOCIALECHO_BASE" != "$SOCIALECHO_HOME" ] && [ -r "$SOCIALECHO_BASE/socialecho-server.jar" ] ; then
  CLASSPATH="$CLASSPATH""$SOCIALECHO_BASE"/socialecho-server.jar
else
  CLASSPATH="$CLASSPATH""$SOCIALECHO_HOME"/socialecho-server.jar
fi

if [ -z "$SOCIALECHO_OUT" ] ; then
  SOCIALECHO_OUT="$SOCIALECHO_BASE"/logs/socialecho.out
fi
...
SOCIALECHO_CONFIG_FILE="$SOCIALECHO_HOME"/conf/server.conf
  ...
  touch "$SOCIALECHO_OUT"
    "$_RUNJAVA" "$LOGGING_CONFIG" $LOGGING_MANAGER $JAVA_OPTS $SOCIALECHO_OPTS \
	  ...
	  -Dtwitterecho.server.home="$SOCIALECHO_SERVER_HOME" \
	  pt.sapo.labs.twitterecho.Main "$@" \
	  -config="$SOCIALECHO_HOME"/conf/server.conf \
  ...
  if [ ! -z "$SOCIALECHO_PID" ]; then
	...
	ps aux | grep socialecho-server | awk '{print $2}' | xargs kill -9
	echo "Done"

else

  echo "Usage: socialecho-server.sh ( commands ... )"
  echo "commands:"
