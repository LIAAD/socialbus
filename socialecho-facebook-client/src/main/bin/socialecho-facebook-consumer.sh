...
if [ "$SOCIALECHO_BASE" != "$SOCIALECHO_HOME" ] && [ -r "$SOCIALECHO_BASE/socialecho-facebook-consumer.jar" ] ; then
  CLASSPATH="$CLASSPATH""$SOCIALECHO_BASE"/socialecho-facebook-consumer.jar
else
  CLASSPATH="$CLASSPATH""$SOCIALECHO_HOME"/socialecho-facebook-consumer.jar
fi

if [ -z "$SOCIALECHO_OUT" ] ; then
  SOCIALECHO_OUT="$SOCIALECHO_BASE"/logs/socialecho-facebook-consumer.out
fi
...
echo "##############################################################################"
  ...
  if [ ! -z "$SOCIALECHO_PID" ]; then
	...
	ps aux | grep socialecho-facebook-consumer | awk '{print $2}' | xargs kill -9
	echo "Done"

else

  echo "Usage: socialecho-facebook-consumer.sh ( commands ... )"
  echo "commands:"
