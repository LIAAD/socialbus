...
if [ "$TWITTERECHO_BASE" != "$TWITTERECHO_HOME" ] && [ -r "$TWITTERECHO_BASE/twitterecho-streaming-client.jar" ] ; then
  CLASSPATH="$CLASSPATH""$TWITTERECHO_BASE"/twitterecho-streaming-client.jar
else
  CLASSPATH="$CLASSPATH""$TWITTERECHO_HOME"/twitterecho-streaming-client.jar
fi

if [ -z "$TWITTERECHO_OUT" ] ; then
  TWITTERECHO_OUT="$TWITTERECHO_BASE"/logs/twitterecho-client.out
fi
...
echo "                                      \/             \/     \/     \/        "
echo "##############################################################################"
echo " TwitterEcho Streaming Client, 2013, Version 0.3"

TWITTERECHO_CONFIGFILE=$2
echo $TWITTERECHO_CONFIGFILE
echo "##############################################################################"
  ...
  touch "$TWITTERECHO_OUT"
    "$_RUNJAVA" "$LOGGING_CONFIG" $LOGGING_MANAGER $JAVA_OPTS $TWITTERECHO_OPTS \
	  ...
	  -Dconfig="$TWITTERECHO_CONFIGFILE" \
      pt.sapo.labs.Main "$@" start \
      >> "$TWITTERECHO_OUT" 2>&1 &
  ...
  if [ ! -z "$TWITTERECHO_PID" ]; then
	...
	echo "Stopping TwitterEcho Client"
	ps aux | grep twitterecho-streaming-client | awk '{print $2}' | xargs kill -9
	echo "Done"

else

  echo "Usage: twitterecho-streaming-client.sh ( commands ... )"
  echo "commands:"
  echo "  start [configfile] Start Twitterecho in a separate window"
