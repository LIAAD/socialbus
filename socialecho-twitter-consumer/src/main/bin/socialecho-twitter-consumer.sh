...
if [ "$SOCIALECHO_BASE" != "$SOCIALECHO_HOME" ] && [ -r "$SOCIALECHO_BASE/socialecho-twitter-consumer.jar" ] ; then
  CLASSPATH="$CLASSPATH""$SOCIALECHO_BASE"/socialecho-twitter-consumer.jar
else
  CLASSPATH="$CLASSPATH""$SOCIALECHO_HOME"/socialecho-twitter-consumer.jar
fi

if [ -z "$SOCIALECHO_OUT" ] ; then
  SOCIALECHO_OUT="$SOCIALECHO_BASE"/logs/socialecho-twitter-consumer.out
fi
...
echo "##############################################################################"
  ...
  if [ ! -z "$SOCIALECHO_PID" ]; then
	...
	ps aux | grep socialecho-twitter-consumer | awk '{print $2}' | xargs kill -9
	echo "Done"

else

  echo "Usage: socialecho-twitter-consumer.sh ( commands ... )"
  echo "commands:"
