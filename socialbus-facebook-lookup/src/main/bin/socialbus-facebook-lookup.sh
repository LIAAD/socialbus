...
if [ "$SOCIALECHO_BASE" != "$SOCIALECHO_HOME" ] && [ -r "$SOCIALECHO_BASE/socialbus-facebook-consumer.jar" ] ; then
  CLASSPATH="$CLASSPATH""$SOCIALECHO_BASE"/socialbus-facebook-consumer.jar
else
  CLASSPATH="$CLASSPATH""$SOCIALECHO_HOME"/socialbus-facebook-consumer.jar
fi

if [ -z "$SOCIALECHO_OUT" ] ; then
  SOCIALECHO_OUT="$SOCIALECHO_BASE"/logs/socialbus-facebook-consumer.out
fi
...
echo "##############################################################################"
echo " SocialBus Streaming Client, 2013, Version 0.5"

SOCIALECHO_ARGS=" "
SOCIALECHO_FILTER_FILE=$2
...
echo "##############################################################################"
  ...
  if [ ! -z "$SOCIALECHO_PID" ]; then
	...
	echo "Stopping SocialBus Client"
	ps aux | grep socialbus-facebook-consumer | awk '{print $2}' | xargs kill -9
	echo "Done"

else

  echo "Usage: socialbus-facebook-consumer.sh ( commands ... )"
  echo "commands:"
  echo "  start [configfile] Start SocialBus in a separate window"
