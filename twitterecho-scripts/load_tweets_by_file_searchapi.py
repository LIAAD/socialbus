...
#user_file = "/tmp/users_stream_portugal_full.csv"
#user_file = "/tmp/users_intersection.txt"
user_file = "tweetsid_jul2013_popstar.txt"

intersect_users_data = []
f =  open(user_file,"r")
for line in f:
    intersect_users_data.append(long(line))

#for uid in intersect_users_data:
#    print uid
...
def parseToJson(json_string):
   # print json_string
    ...
    print json_tweet["created_at"],str(json_tweet["id"])
    
    if long(json_tweet['id']) in intersect_users_data :
        print "match ",json_tweet['id']
        #intersec_user = True
...
def indexTweetIntoSolr(json):
    ...
    try:
        ...
        version='12',
        datasource_s="search_api_popstar"
        )
