...
def load_tweets_from_page(current_page):
    ...
    collection = mongo_conn.get_collection("tweets")
    start_date = datetime.strptime('Jun 10 2012', '%b %d %Y')
    end_date = datetime.strptime('Oct 18 2012', '%b %d %Y')
...
def count_total_rows():
    ...
    total = tweets_info_col.find().count()
