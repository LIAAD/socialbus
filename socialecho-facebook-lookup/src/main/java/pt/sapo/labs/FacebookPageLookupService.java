...
public class FacebookPageLookupService implements Runnable
{
    ...
    private Boolean considerOnlyVerifiedPages;
    private Integer considerPageWithMinFanCount = 0;

    public FacebookPageLookupService(CompositeConfiguration config){
        this.config = config;

        this.considerOnlyVerifiedPages = config.getBoolean("consider.only.verified.pages");
        this.considerPageWithMinFanCount = config.getInt("consider.page.with.min.fan_count",0);
    ...
    public void run() {
        ...
        String outputFilePath = config.getString("output.file","lookup_pages.csv");
...
//            logger.error("Error to search by users",e);
    ...
    private FacebookPage findBestPage(List<FacebookPage> pages){
        ...
        FacebookPage bestPage = null;

        for(FacebookPage page : pages){

            if(this.considerOnlyVerifiedPages){
                if(page.isIs_verified()){
                    return page;
                }
            }else{
                if(this.considerPageWithMinFanCount > 0){

                    if(page.isIs_verified()){
                        return page;
                    }

                    if(page.getFan_count() > this.considerPageWithMinFanCount){

                        if(page.getFan_count() > bestPage.getFan_count()){
                            bestPage = page;
                        }
                    }
