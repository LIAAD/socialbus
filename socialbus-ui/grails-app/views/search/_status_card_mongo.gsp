<div class="twt-border" style="margin-bottom: 20px !important;">
	...
	<div class="h-card p-author">      
		<a class="screen-name u-url" href="https://twitter.com/${status.user.screen_name}" data-screen-name="${status.user.screen_name}">
			<span class="avatar">
				<img src="${status.user.profile_image_url}" class="u-photo" alt=""></span>
			<span class="p-name">${status.user.name}</span>
			<span class="p-nickname">@<b>${status.user.screen_name}</b></span>
		</a>
	...
	<div class="footer">
		<a class="view-details" href="https://twitter.com/${status.user.screen_name}/statuses/${status.id}"><span class="dt-updated " title="${status.created_at}">
